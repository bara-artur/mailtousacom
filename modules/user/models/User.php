...
class User extends ActiveRecord  implements IdentityInterface
{
   ...
   * @return путь(string)
   */
  public function getUserPath($id = false, $test = true)
  {
    if (!$id) {
      $id = $this->id;
    }
  ...
  public function delFile($key)
  {
    ...
    //echo ($_SERVER['DOCUMENT_ROOT'].'/'.$path. $key);
  ...
  public function loadDoc($files)
  {
    ...
    if ($count_now - 2 >= 5) {
      ...
      foreach ($files as $file) {
        $file_name = ($path . date('Ymd_His')  . '.' . $file->extension);
